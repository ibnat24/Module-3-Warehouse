...
```
📁 your-folder/
    algorithm.py   ← the script
    order_itemtypes.csv              ← randomly generated (item types per order)
    order_quantities.csv             ← randomly generated (quantities per order)
    orders_totes.csv                 ← randomly generated (tote IDs per order)
```

The three CSV files are generated by the course-provided random data generator. Regenerate them to get a new instance — the script will automatically adapt.
...
**Section 1 — Heuristic Comparison Table**
...
**Section 2 — Complete Demo Instructions**  
...
**Section 3 — Random Instance Benchmark**  
...
**File output:**
- `conveyor_input_LPT_PENALTY.csv` — upload this to the IDEAS Clinic conveyor system
...
### Decision 1 — Tote Sequence
*Which tote do you physically pick up and load first?*

| Heuristic | Logic |
|---|---|
| Baseline | Load totes in ascending tote ID order |
| **EOCF** (Earliest Order Completion First) | Load totes that serve orders needing the fewest total totes first — those orders can complete as soon as their one tote is processed |
| MIPTF (Most Items Per Tote First) | Load totes that serve the most simultaneous orders first |

### Decision 2 — Lane Assignment
*Which order goes in which bin?*

| Heuristic | Logic |
|---|---|
| Baseline | Assign by order ID mod 4 (order 0→lane 0, order 1→lane 1, etc.) |
...
### Decision 3 — Item Placement Sequence
...
This combination achieves the **theoretical minimum makespan** on the demo instance and wins on **50 out of 50** random benchmark instances with an average saving of **17.7 seconds (−12.6%)** vs. baseline.
...
| `FULL_LOOP` | 24s | Time for one full belt circulation (4 × 6s) |
...
`completion(order) = tote_load_time + lane × TRAVEL_TIME`  
where `tote_load_time = tote_position × FULL_LOOP`
...
ITEM_PLACE_TIME = 2    # seconds to place one item — adjust if timed at demo station
TRAVEL_TIME     = 6    # seconds between scanners — fixed by physical system
```
